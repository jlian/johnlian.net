...
JetKVM is a great device. It already has a Wake-on-LAN button built in. I went a step further and set up an nginx reverse proxy in front of it[^nginx] that fires a WoL packet on every single request via `mirror` and a CGI script. Every page load, every API call, every asset fetch sends another magic packet. Opening the JetKVM dashboard effectively carpet-bombs the PC with wake requests. This is completely unnecessary and I did it mostly because it's funny. But the one thing JetKVM couldn't do was wake my PC over USB, the same way a real keyboard would. [Issue #120](https://github.com/jetkvm/kvm/issues/120) had been open for a while with community interest, and since I'd [already solved this exact problem](https://johnlian.net/posts/tinypilot-usb-wake/) on different hardware, I figured I should take a crack at it.

[^nginx]: The nginx config uses `mirror /wake-trigger` on the `location /` block, which means every proxied request also hits a FastCGI endpoint that runs a WoL script. The JetKVM web UI makes dozens of requests on load (assets, API calls, WebSocket upgrade), so a single page open sends a barrage of wake packets. WebRTC video doesn't work through the proxy (needs direct UDP), but the controls do, and the auto-wake means the PC is usually already booting by the time the page finishes loading.
