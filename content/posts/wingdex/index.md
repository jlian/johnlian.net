...
What stuck was the editorial look and the nod to *Wingspan*,[^wingspan] and today the icon is a green bird with nothing about it that says glass cleaner.[^logo]

[^logo]: I tried to get AI to draw the logo first. As I remember it, the agent (GPT-5.3) drew one, reviewed its own work, and decided it looked good apart from the wings being a bit too pointy, and when I opened the SVG I couldn't tell whether it was meant to be a bird. So I drew it myself in Inkscape, the app icon plus outline and filled versions of a custom SF Symbol, starting on March 5 with asking ChatGPT how to get the offset gap Apple's symbols leave between shapes (build a separator path, thicken it and subtract it, which it called a "knockout gap") and ending, as far as the repo can tell, with an app icon rebuilt from Icon Composer layers on March 23. I'm sorry, but Inkscape is not good, and I should have just paid for Adobe Illustrator. <!-- TODO(John): how many hours the icon took ("ungodly"); add the number if you have one. --> <!-- TODO(John): HN has a lot of FOSS people; a softer option is "Inkscape and I did not get along, and I should have just paid for Illustrator." Your call. --> <!-- TODO(John): consider cropping inkscape-knockout.png (in the bundle, not embedded) to just your in-progress bird (left tile, with Inkscape nodes) as a small inline image; the other two tiles are Apple's SF Symbols. -->
